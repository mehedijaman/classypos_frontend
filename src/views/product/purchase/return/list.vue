    ...
    <div class="col-md-12">
      <div class="card">
        <div class="col-md-12 header" style="margin-bottom:10px;">
          ...
          <router-link :to="{ path: '/Purchase/Return/New' }"><button v-if="NewPurchaseReturnPermission" class="btn btn-success pull-right">New Return</button></router-link>
        </div>
  ...
  data () {
    return {
      table: {
        ...
        subTitle: 'Recently Returned Purchases.',
        columns: [...tableColumns]
      },
      NewPurchaseReturnPermission: true,
    ...
    axios.get(`http://techlab.classy.pos/Api/User/Permission/PurchaseOrderList?token=` + Token)
      .then((response) => {
        ...
        if (response.data === 'Empty Array') {
          ...
          this.PurchaseOrderList()
          this.AuthCheck()
          this.PermissionCheck('PurchaseReturnDetails')
          this.PermissionCheck('PurchaseReturnDelete')
          this.PermissionCheck('NewPurchaseOrder')
        }
      })
      .catch(() => {
  ...
  methods: {
    ...
    PermissionCheck (RoleName) {
      ...
      axios.get(`http://techlab.classy.pos/Api/User/Permission/` + RoleName + `?token=` + Token)
        .then((response) => {
          if (RoleName === 'PurchaseReturnDetails') {
            if (response.data === 'Empty Array') {
          ...
          if (RoleName === 'PurchaseReturnDelete') {
            if (response.data === 'Empty Array') {
    ...
    PurchaseOrderList () {
      axios.get(`http://techlab.classy.pos/Api/V1/Purchase/Return/List`)
        .then((response) => {
    ...
    deletePurchaseOrder (product) {
      this.DeletePurchaseReturnID = product.id
      this.$swal({
        title: 'Are you sure?',
    ...
    detailsProduct (id) {
      axios.get('http://techlab.classy.pos/Api/V1/Purchase/Return/Details/' + id)
        .then((resp) => {
