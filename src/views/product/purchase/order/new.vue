...
export default {
  ...
  created () {
    ...
    axios.get(`http://techlab.classy.pos/Api/User/Permission/PurchaseNew?token=` + Token)
      .then((response) => {
    ...
    PurchaseOrder () {
      ...
      for (var i = 0; i < this.FirstCostPrice.length; i++) {
        if (this.FirstQuantity[i] > 0) {
          this.testPurchase.OrderedProducts.push({'ProductID': this.ProductInformation[i], 'Description': this.ProductDescription[i], 'Model': this.ModelNumber[i], 'Price': this.FirstCostPrice[i], 'Qty': this.FirstQuantity[i]})
        }
      }
